...
| K-08 | 项目范围 | P2 | 已实现（v0.03） | 敌机齐射 110ms 错峰发射；敌弹改为"发射瞬间瞄准直射"（经典 STG），精英机 5 发扇形弹幕。Boss 弹幕仍未实现。 |

## 当前限制与待办

| ID | 优先级 | 状态 | 说明与建议 |
|---|---|---|---|
| K-06 | P2 | 已实现（v0.04） | 程序化 Web Audio 音效（开火/爆炸/受击/拾取/结算/UI）+ 程序化 BGM 循环（Am-F-C-G 音序器），均由 `client/src/game/audio.ts` 合成，无外部资源。首次手势解锁（菜单启动或场景首次按键/点击），M 键静音开关一并生效。 |
| K-07 | P2 | 已知限制 | 项目是浏览器横屏版本；竖屏仅显示横置提示，不含原生桌面或移动端打包。 |
| K-09 | 项目范围 | P2 | 已实现（v0.03） | 精英扇形弹幕（5 发、±20°、略慢）。走位仍为直线+正弦摆动，无独立 AI。 |
| K-10 | P2 | 部分已解决（v0.05.2） | Latin 字体（Barlow Condensed / IBM Plex Mono）已自托管到 `client/src/assets/fonts/`；Noto Sans SC（CJK 子集过多）仍走 Google Fonts，离线回退系统字体。 |
