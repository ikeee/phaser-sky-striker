...
export class BattleScene extends Phaser.Scene {
  ...
  private updateEntities() {
    const clean = (group: Phaser.Physics.Arcade.Group, limit: number) => {
      group.children.forEach((child) => {
        const sprite = child as Ship;
        if (sprite.active && (sprite.y < -80 || sprite.y > GAME_HEIGHT + 90 || sprite.x < -90 || sprite.x > GAME_WIDTH + 90)) {
          sprite.disableBody(true, true);
        }
      });
    };
    clean(this.playerBullets, -60);
    clean(this.enemyBullets, GAME_HEIGHT + 80);
    clean(this.enemies, GAME_HEIGHT + 90);
    clean(this.pickups, GAME_HEIGHT + 90);
  }
  ...
  private fireEnemyVolley() {
    ...
    this.enemies.children.forEach((child) => {
      ...
      bullet.setDisplaySize(14, 14).setVelocityY(300).setDepth(7);
  ...
  private updateHud() {
    this.scoreText.setText(this.score.toString().padStart(6, "0").replace(/(\d{3})(\d{3})/, "$1 $2"));
